...
export function financeForecast(yearParam?: number, incomeParam?: number, incomesParam?: string,
  depPriceParam?: number, depMethodParam?: string) {
  const d = db();
  ...
  // VAT-exempt passive income (GmbH investment/participation income) added at
  // full value; plus the forward salary scenario (net override + sick-leave
  // month) so the Forecast honours exactly the same assumptions as the plan.
  const passiveByMonth: Record<string, number> = {};
  for (const [k, v] of Object.entries(prefPath<Record<string, unknown>>("forecast.passiveByMonth", {}) || {})) {
    const n = Number(v); if (Number.isFinite(n) && String(v).trim() !== "") passiveByMonth[k] = n;
  }
  const planY = prefPath<Record<string, unknown>>(`payrollPlan.${year}`, {}) || {};
  const salNetOverride = String((planY as any)._netSalary ?? "").trim() !== "" ? Number((planY as any)._netSalary) : null;
  const salSickFrom = Number((planY as any)._sickFrom ?? 0) || 0;
  ...
  for (let i = 0; i < months; i++) {
    ...
    const b = buckets[key] ?? { obligations: 0, bills: 0, items: [] };
    const monthNet = salNetOverride != null ? salNetOverride : netSalary;
    let pay = (!empStart || mIso >= `${empStart.slice(0, 7)}-01`) ? monthNet : 0;
    if (salSickFrom && mYear === year && mNo >= salSickFrom) pay = 0;   // paid sick leave → KTG carries the salary
    ...
    const passive = passiveByMonth[key] ?? 0;
    const inc = (perMonthIncome[key] ?? incomeM) + passive;
    const out = round2(pay + b.obligations + b.bills + res);
    const net = round2(inc - out);
    cashBal = round2(cashBal + net);
    const row = { key, label: `${MONTH_ABBR[mNo]} ${mYear}`, income: round2(inc), passive: round2(passive),
  ...
  // Vehicle depreciation reduces taxable profit. Prefer booked vehicles'
  // year-`year` depreciation; with none booked, fall back to the projection
  // stored in Prefs (vehicle.projPrice/projMethod), overridable by the
  // dep_price/dep_method query params — so the impact is visible before the
  // car is booked, exactly like the classic Reports card.
  let projPrice: number | null = null, projMethod = "degressive_40";
  const vp = prefPath<Record<string, unknown>>("vehicle", {}) || {};
  if (vp.projPrice != null && String(vp.projPrice).trim() !== "") projPrice = Number(vp.projPrice);
  if (vp.projMethod) projMethod = String(vp.projMethod);
  if (depPriceParam != null) { projPrice = depPriceParam; if (depMethodParam) projMethod = depMethodParam; }

  const vehs = d.prepare("SELECT purchase_date, purchase_price, depreciation_method FROM vehicles WHERE is_active=1").all() as any[];
  let vehDep = 0; let vehDepSource: "booked" | "projection" | "none" = "none";
  if (vehs.length) {
    for (const v of vehs) {
      const p = Number(v.purchase_price || 0);
      const pyear = v.purchase_date ? Number(String(v.purchase_date).slice(0, 4)) : year;
      const n = Math.max(0, year - pyear);
      if ((v.depreciation_method || "degressive_40") === "degressive_40") vehDep += p * Math.pow(0.6, n) * 0.40;
      else vehDep += Math.max(0, Math.min(p * 0.20, p - p * 0.20 * n));
    }
    vehDepSource = "booked";
  } else if (projPrice) {
    vehDep = projPrice * (projMethod === "degressive_40" ? 0.40 : 0.20);
    vehDepSource = "projection";
  }
  vehDep = round2(vehDep);

  let projRevNet = 0, passiveTotal = 0; const projMonths: string[] = [];
  for (const r of rows) {
    const mNo = Number(r.key.slice(5, 7));
    passiveTotal += r.passive ?? 0;
    if (invMonths.has(mNo)) continue;
    const vatable = r.income - (r.passive ?? 0);            // invoice income carries VAT
    projRevNet += vatable / (1 + VAT) + (r.passive ?? 0);   // passive is VAT-exempt, full value
  ...
  if (netSalary && year >= now.getFullYear()) {
    ...
    const lastMonth = salSickFrom ? salSickFrom - 1 : 12;   // salary stops at sick leave
    payrollProjMonths = Math.max(0, Math.min(12, lastMonth) - first + 1);
  }
  ...
  const pbt = round2(invNetY + otherY + projRevNet - payRows.t - payrollProj - billsY - billsProj - fiduciary - vehDep);
  ...
  return {
    pl: {
      ...
      vehicle_depreciation: vehDep, vehicle_depreciation_source: vehDepSource,
      passive_income: round2(passiveTotal),
