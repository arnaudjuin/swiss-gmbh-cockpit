// One-click accountant export — shared by /api/reports/accountant-package
// and the public /share/{token}/zip endpoint.
import fs from "fs";
import path from "path";
import JSZip from "jszip";
import { db, MONTH_NAME } from "./db";
import { DIRS } from "./files";
import { csvRow, pyFloat } from "./pycsv";
import { typeLabel } from "./obligations";
import { companyName } from "./biz";
...
export async function buildAccountantPackage(year: number): Promise<{ buf: Buffer; filename: string }> {
  const zip = new JSZip();
  ...
  return { buf, filename: `${companyName()} Accountant Package ${year}.zip` };
}
