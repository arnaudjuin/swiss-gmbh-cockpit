...
// The single company name for all outward-facing output (exports, PDFs,
// share pages, ICS, the AI prompt). Prefers the sidebar's app.companyName,
// then the invoicing business settings, then the packaged default.
export function companyName(): string {
  const explicit = prefPath<string>("app.companyName", "");
  return explicit && explicit.trim() ? explicit.trim() : bizSettings().company;
}
