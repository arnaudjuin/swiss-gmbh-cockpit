...
export function Stat({ label, value, mod = null, hint, span }: {
  label: string; value: string; mod?: Accent; hint?: React.ReactNode; span?: number;
}) {
  return (
    <div className={`stat${mod ? ` stat--${mod}` : ""}`}
      style={span && span > 1 ? { gridColumn: `span ${Math.min(span, 3)}` } : undefined}>
