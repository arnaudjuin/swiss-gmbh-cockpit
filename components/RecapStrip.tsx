...
// Each tile links to the matching Next page (in-app), the way the classic
// navigateTo() jumps to that page within its own SPA.
...
export default function RecapStrip({ ov, reserves, runway, forecast, dividends }: {
  ...
  return (
    <div className="recap-grid">
      {p.bank ? (
        <RecapTile href="/bank" label={`🏦 Bank · ${p.bank.bank}`} value={chf(p.bank.closing)}
        ...
        <RecapTile href="/bank" label="🏦 Bank" value="—" hint="Upload your first statement" />
      )}

      <RecapTile href="/cash" label="💰 Cash & reserves" value={chf(earmarked)}
      ...
      <RecapTile href="/bills" label="🧾 Bills" value={chf(p.bills.total)}
      ...
      <RecapTile href="/obligations" label={`📋 Obligations ${o.year}`} value={chf(o.unpaid)}
      ...
      <RecapTile href="/payroll" label="👤 Payroll"
      ...
      <RecapTile href="/invoices" label="📄 Invoices" value={chf(p.receivables.total)}
      ...
      <RecapTile href="/bank" label="🔁 Kontokorrent" value={chf(Math.abs(k.net))}
      ...
      <RecapTile href="/reports" label="🏛 VAT" value={chf(p.vat.open_obligations)}
        mod={p.vat.open_obligations > 0 ? "warn" : null}
        hint={`to remit · collected ${chf(p.vat.collected_year)} on invoices this year`} />

      {dividends && (
        <RecapTile href="/dividends" label={`⚡ Dividends FY ${new Date().getFullYear()}`}
      ...
      {forecast && low && (
        <RecapTile href="/forecast" label="📈 Forecast" value={chf(low.cash_end)}
      ...
      <RecapTile href="/expenses" label="🧳 Expenses & trips" value={chf(k.reports_open)}
