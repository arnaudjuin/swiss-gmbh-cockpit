...
export default function ForecastPage() {
  ...
  return (
    <div className="page active">
      ...
      {fc.pl && (
        <div className="panel" style={{ padding: "12px 16px", marginBottom: 16 }}>
          ...
          <div className="hint" style={{ marginTop: 6 }}>
            ...
            {" "}({fc.pl.projected_months.length} months from the incomes above)
            {fc.pl.passive_income > 0 && <> <span className="hint">incl. {chf(fc.pl.passive_income)} passive · VAT-exempt</span></>}
            ...
            {fc.pl.vehicle_depreciation > 0 && <>
              {" "}− vehicle depreciation {chf(fc.pl.vehicle_depreciation)}
              {" "}<span className={`chip ${fc.pl.vehicle_depreciation_source === "booked" ? "chip--ok" : "chip--expected"} chip--sm`}>{fc.pl.vehicle_depreciation_source}</span></>}
          </div>
          ...
          {fc.pl.note && <div className="hint hint--sm" style={{ marginTop: 4 }}>{fc.pl.note}</div>}
        </div>
      )}
      ...
      <h3 style={{ margin: "0 0 4px" }}>Month-by-month plan</h3>
      <p className="hint" style={{ margin: "0 0 10px" }}>
        Income in, everything that leaves the bank, and the running net — per month. Edit the income right on each card.
        Honors the salary/sick-leave scenario and the pre-fund ceiling.
      </p>
      <MonthByMonth year={fc.year} />
