...
import { AED_TO_CHF } from "@/server/expenseReports";
import { companyName } from "@/server/biz";
...
export const GET = guard(async (req: NextRequest, ctx: any) => {
  ...
  ws.mergeCells("A1:F1");
  ws.getCell("A1").value = `${companyName()} - Travel Expenses ${periodLabel}`;
  ...
  return new NextResponse(new Uint8Array(buf), { headers: {
    ...
    "Content-Disposition": contentDisposition(`Travel Expenses ${periodLabel} ${companyName()} 101119.LOD-SW_GCS-24032.xlsx`),
  } });
});
