...
export const GET = pub(async (_req: NextRequest, ctx: any) => {
  ...
  const rows: any[] = db().prepare("SELECT * FROM expenses ORDER BY expense_date DESC").all();
  let out = csvRow(["Date", "Description", "Category", "Amount (CHF)", "Original", "Original Currency"]);
  for (const r of rows)
    out += csvRow([r.expense_date, r.description, r.category, pyFloat(r.amount),
      r.original_amount ? pyFloat(r.original_amount) : "", r.original_currency || ""]);
  return csvResponse(out, "expenses.csv");
});
