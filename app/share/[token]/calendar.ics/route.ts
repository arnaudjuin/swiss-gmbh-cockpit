...
export const GET = pub(async (_req: NextRequest, ctx: any) => {
  ...
  if (totalTarget > 0) {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      events.push(event(`cockpit-contrib-${ym}@cockpit`, `${ym}01`,
        ...
        `Monthly budget contribution day. Open ${companyName()} → Budget Balances → Contribute All.`,
        "Budget", 0));
      m += 1;
      if (m > 12) { m = 1; y += 1; }
    }
  ...
  for (const [d, period] of vat) {
    events.push(event(`cockpit-vat-${d}@cockpit`, d.replace(/-/g, ""),
      ...
      `Quarterly VAT filing due. Open ${companyName()} → Reports → VAT Tracker.`, "VAT", 14));
  }

  const ics = [
    "BEGIN:VCALENDAR", "VERSION:2.0",
    `PRODID:-//${companyName()}//Finance//EN`,
    "CALSCALE:GREGORIAN", "METHOD:PUBLISH",
    `X-WR-CALNAME:${companyName()} Finance`,
