...
import { loadPrefs, pref, divTax, type DivTax } from "@/lib/prefs";
...
export default function DividendsPage() {
  ...
  useEffect(() => { loadPrefs().then(p => { setDt(divTax(p)); setPlan(pref<Plan | null>(p, "dividends", null)); }); }, []);
        ...
        <>
          <div className="stats-grid">
            <Stat label="Gross pot (all years)" value={chf(gross)} mod="owner"
              hint={`WHT ${(dt.wht * 100).toFixed(0)}% is timing only — refunded when declared`} />
            <Stat label="Net after income tax" value={chf(net)} mod="ok"
              hint={`≈${(eff * 100).toFixed(1)}% effective (${(dt.fedIncl * 100).toFixed(0)}% × ${fed}% fed + ${(dt.cantIncl * 100).toFixed(0)}% × ${cant}% cantonal)`} />
